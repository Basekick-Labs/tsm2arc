...
4. **On resume**, for each `(sourceDB, shard)`, read its last committed `chunkSeq` **and its cursor** — the `(raw series key, timestamp)` of the last line in that chunk, stored atomically with every commit (0.1.5+). Because emission order is deterministic (series key ascending, timestamps strictly ascending and unique within a series), the cursor splits the shard's output exactly: extraction **seeks** to it — series before the cursor are never read, and index-level block pruning skips everything at or before its timestamp — then POSTing continues at `chunkSeq+1` with byte-identical chunk numbering. A cursor series missing from the shard aborts loudly (source data changed). Checkpoints written before 0.1.5 have no cursor: those shards fall back to re-deriving chunks from the start and skipping `≤ chunkSeq` without POSTing.
5. **Crash window:** the only overlap risk is a chunk that Arc *received and persisted* but whose 2xx we never recorded (crash between Arc's commit and our checkpoint write). On resume we re-POST that one chunk → duplicate rows for that chunk only.
   ...
   - For **tagless series**: the duplicate persists. Bounded to **at most `--inflight` chunks per shard** (the in-flight ones at crash time — one at the default), and only on an unclean crash or a mid-flight send failure with later chunks already accepted. Acceptable and documented; an optional post-migration verification (§8) detects it.
...
Per-chunk is the natural atomic unit because the chunk is what Arc accepts atomically. Per-series checkpointing would require Arc-side partial-chunk acknowledgement, which the import endpoint does not provide (it's all-or-nothing per request). Per-chunk keeps the checkpoint table tiny (one row per shard) and the resume logic trivial. The seek cursor gives per-chunk checkpointing per-line resume *granularity* anyway, without changing the atomic unit.
...
### Concurrent sends (`--inflight`)
Up to K POSTs per shard fly concurrently, but commits apply in STRICT sequence order: commit(n) only after every seq ≤ n succeeded, so the checkpoint's committed prefix always equals the fully acked prefix and resume stays exact at any K. Failure of seq n after n+1..n+K−1 were accepted leaves those later chunks in Arc uncommitted — the resume re-sends them, which is the ≤K-chunk duplicate bound above. Chunk ARRIVAL order at Arc was never a correctness input (append-only ingest, partitioning by data time, dedupe on (tags, ts) at compaction), so out-of-order landing is safe. Error results flow through the same ordered path as successes — a seq whose result never arrived would deadlock the drain barrier, so workers always deliver one result per job.
